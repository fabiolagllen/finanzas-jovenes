...
(function(){
  ...
  function ensureReminders(){
    if(isProfile()||document.getElementById('pagos'))return;const main=document.querySelector('main');if(!main)return;
    const section=document.createElement('section');section.id='pagos';section.className='fj-reminders';
    ...
    const cta=main.querySelector('.cta')?.closest('section');if(cta)main.insertBefore(section,cta);else main.appendChild(section);
  }

  window.navigateToSection=function(id){goSection(id)};
  ...
  function build(){
    ...
    const user=document.createElement('div');user.className='fj-menu-user';const auth=document.createElement('button');auth.type='button';user.appendChild(auth);nav.replaceChildren(logo,links,user);
    let mobile=document.querySelector('.fj-mobile-nav');if(!mobile){mobile=document.createElement('nav');mobile.className='fj-mobile-nav';mobile.setAttribute('aria-label','Navegación móvil');document.body.appendChild(mobile)}
