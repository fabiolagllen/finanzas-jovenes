...
(function(){
  ...
  function isProfile(){
    ...
    return p.endsWith('/profile.html')||p.endsWith('/profile');
  }
  function currentId(){
  ...
  function goSection(id){
    if(isProfile()){ location.href='./index.html#'+id; return; }
  ...
  function styles(){
    ...
    const s=document.createElement('style');s.id='fjSharedDesktopMenu';
    s.textContent=`
      ...
      body>header .fj-menu-links a{display:flex!important;align-items:center!important;color:#554D53!important;text-decoration:none!important;margin:0!important;padding:12px 13px!important;border-radius:15px!important;font-size:.91rem!important;font-weight:700!important;transition:.22s!important;border:1px solid rgba(255,255,255,.025)!important;background:rgba(255,255,255,.018)!important;box-shadow:0 5px 14px rgba(0,0,0,.10)!important}
      body>header .fj-menu-links a:hover,body>header .fj-menu-links a.active{color:#7A1F3D!important;background:rgba(57,255,136,.105)!important;border-color:rgba(57,255,136,.20)!important}
  ...
  function authClient(){
    ...
    if(window.supabase?.createClient){try{return window.__fjSharedSupabase=window.__fjSharedSupabase||window.supabase.createClient('https://pgfgmxeqisrwkrwtvbum.supabase.co','sb_publishable_6al9XSM0nTc6-RlkX2UUrw_SLaQdwEh')}catch(e){}}
    return null;
  }
  ...
  function loginAction(){
    ...
    sessionStorage.setItem('fj_open_login','1');location.href='./index.html';
  }
  ...
  async function syncAuth(){
    const client=authClient();if(!client?.auth){updateAuthButton(null);return}
    try{const {data}=await client.auth.getSession();updateAuthButton(data?.session||null);client.auth.onAuthStateChange((_event,session)=>updateAuthButton(session))}catch(e){updateAuthButton(null)}
  }

  function forceRemindersHub(){
    if(isProfile()) return;
    const load=()=>{
      const sec=document.getElementById('pagos');
      if(!sec) return;
      if(window.__fjRemindersHubBooted) delete window.__fjRemindersHubBooted;
      const old=document.querySelector('script[data-fj-reminders-force]');
      if(old) old.remove();
      const s=document.createElement('script');
      s.src='./reminders-hub.js?v=2&force='+Date.now();
      s.dataset.fjRemindersForce='true';
  ...
  function build(){
    ...
    const user=document.createElement('div');user.className='fj-menu-user';const auth=document.createElement('button');auth.type='button';user.appendChild(auth);
    nav.replaceChildren(logo,links,user);
    let mobile=document.querySelector('.fj-mobile-nav');
    if(!mobile){mobile=document.createElement('nav');mobile.className='fj-mobile-nav';document.body.appendChild(mobile)}
    mobile.replaceChildren();
    ITEMS.forEach(item=>{const b=document.createElement('button');b.type='button';b.dataset.target=item.id;b.innerHTML='<span>'+item.icon+'</span>'+item.label;b.addEventListener('click',()=>navigate(item.href));mobile.appendChild(b)});
    setActive();updateAuthButton(null);syncAuth();forceRemindersHub();
    window.__fjSharedMenuBooted=true;
    window.__fjSharedMenuStarting=false;
  }
  function start(){
    build();
    ...
    const observer=new MutationObserver(()=>{
      ...
      if(!header||!links||!mobile||links.children.length!==ITEMS.length)build();
    });
