...
(function(){
  ...
  style.textContent=`
    ...
    .fj-badges{max-width:950px;margin:0 auto 34px}.fj-badges h3{text-align:center;margin-bottom:14px}.fj-badges-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:10px}.fj-badge{padding:14px 8px;border-radius:16px;text-align:center;background:#101812;border:1px solid #263a2c;transition:.2s}.fj-badge.unlocked{border-color:#39ff88;box-shadow:0 0 18px rgba(57,255,136,.09)}.fj-badge.locked{opacity:.48;filter:grayscale(.75)}.fj-badge-icon{font-size:1.55rem}.fj-badge-name{font-size:.76rem;font-weight:800;margin-top:5px}.fj-badge-state{font-size:.66rem;color:#91a398;margin-top:3px}
    ...
    @media(max-width:900px){.fj-games-grid{grid-template-columns:1fr}.fj-badges-grid{grid-template-columns:repeat(2,1fr)}.fj-games-wrap{padding:55px 20px}}
  `;
  ...
  section.innerHTML=`
    ...
    <div class="fj-badges"><h3>🏅 Mis logros</h3><div class="fj-badges-grid" id="fjBadgesGrid"></div></div>
    <div class="fj-games-grid">
      ...
      <article class="fj-game"><div class="fj-game-icon">🪙</div><h3>Atrapa el ahorro</h3><p class="fj-game-desc">Haz clic en las monedas antes de que termine el tiempo y suma puntos.</p><span class="fj-game-tag">Rápido · 15 segundos · +1 pt/moneda</span><br><button class="fj-game-btn" onclick="fjCoinStart()">Jugar</button><div id="fjCoinArea" class="fj-game-area fj-coin-game"><div class="fj-score" id="fjCoinScore">Puntos: 0 · Tiempo: 15</div><div class="fj-coins" id="fjCoins"></div></div></div></article>
    </div>`;
  ...
  const badges=[
    {key:'first_game',icon:'🥉',name:'Primer juego',desc:'Completa tu primer juego'},
    {key:'fifty_points',icon:'💰',name:'50 puntos',desc:'Consigue 50 puntos'},
    {key:'three_games',icon:'🔥',name:'Racha inicial',desc:'Juega 3 veces'},
    {key:'organized',icon:'📊',name:'Organizado',desc:'Llega a 150 puntos'},
    {key:'expert',icon:'🏆',name:'Experto financiero',desc:'Llega a 300 puntos'}
  ];
  let gamePoints=0,gamePlayedCount=0;
  ...
  function renderBadges(unlocked=[]){const box=document.getElementById('fjBadgesGrid');if(!box)return;const set=new Set(unlocked);box.innerHTML=badges.map(b=>`<div class="fj-badge ${set.has(b.key)?'unlocked':'locked'}"><div class="fj-badge-icon">${set.has(b.key)?b.icon:'🔒'}</div><div class="fj-badge-name">${b.name}</div><div class="fj-badge-state">${set.has(b.key)?'Desbloqueado':b.desc}</div></div>`).join('');}
  async function loadProgress(){try{renderBadges([]);if(!window.supabaseClient?.auth)return;const {data:{session}}=await window.supabaseClient.auth.getSession();if(!session)return;const {data}=await window.supabaseClient.from('game_progress').select('points,games_played').eq('user_id',session.user.id).maybeSingle();gamePoints=data?.points||0;gamePlayedCount=data?.games_played||0;const {data:earned}=await window.supabaseClient.from('game_badges').select('badge_key').eq('user_id',session.user.id);renderBadges((earned||[]).map(x=>x.badge_key));renderLevel();}catch(e){renderLevel();}}
  async function unlockEligibleBadges(){try{const {data:{session}}=await window.supabaseClient.auth.getSession();if(!session)return;const earnedRows=await window.supabaseClient.from('game_badges').select('badge_key').eq('user_id',session.user.id);const earned=new Set((earnedRows.data||[]).map(x=>x.badge_key));const eligible=[];if(gamePlayedCount>=1)eligible.push('first_game');if(gamePoints>=50)eligible.push('fifty_points');if(gamePlayedCount>=3)eligible.push('three_games');if(gamePoints>=150)eligible.push('organized');if(gamePoints>=300)eligible.push('expert');for(const key of eligible){if(!earned.has(key))await window.supabaseClient.from('game_badges').insert({user_id:session.user.id,badge_key:key});}const {data:updated}=await window.supabaseClient.from('game_badges').select('badge_key').eq('user_id',session.user.id);renderBadges((updated||[]).map(x=>x.badge_key));}catch(e){console.warn('No se pudieron actualizar los logros',e);}}
  async function awardPoints(amount){gamePoints+=amount;gamePlayedCount+=1;renderLevel();try{const {data:{session}}=await window.supabaseClient.auth.getSession();if(!session)return;const {data:old}=await window.supabaseClient.from('game_progress').select('points,games_played,games_won').eq('user_id',session.user.id).maybeSingle();const payload={user_id:session.user.id,points:gamePoints,games_played:(old?.games_played||0)+1,games_won:(old?.games_won||0)+1,updated_at:new Date().toISOString()};gamePlayedCount=payload.games_played;await window.supabaseClient.from('game_progress').upsert(payload,{onConflict:'user_id'});await unlockEligibleBadges();}catch(e){console.warn('No se pudo guardar el progreso',e);}}
  loadProgress();
